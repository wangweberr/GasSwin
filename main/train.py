...
results = {
       ...
       #'pretrained':'/home/chenli/weber/GasSwin/pretrained/swin_tiny_patch244_window877_kinetics400_1k.pth',
        'pretrained':'/home/chenli/weber/GasSwin/main/checkpoints/models/final_model--loss0p150--dice0p744.pth',
        'cache_file':'/home/chenli/weber/GasSwin/main/checkpoints/preprocessed/preprocessed_data-1.pt',
        # 视频处理相关字段
...
def main():
    ...
    model=SwinTransformer3D(
                 ...
                 frozen_stages=results['frozen_stages'],use_checkpoint=True,
    ...
    #定义学习率调度器
    if results['frozen_stages']!=-1:
        warmup = LinearLR(optimizer, start_factor=0.1, total_iters=5)
        cosine  = CosineAnnealingLR(optimizer, T_max=results['frozen_epochs']-5, eta_min=4e-4)
    ...
    for epoch in range(results['epochs']):
        ...
        if epoch==results['frozen_epochs']:
            ...
            cos2  = CosineAnnealingWarmRestarts(optimizer,T_0=8,T_mult=2,eta_min=2e-4)
        ...
        if is_master_process:
            for i, param_group in enumerate(optimizer.param_groups):
                logging.info(f"Epoch {epoch}  group{i} lr={param_group['lr']:.6e}")
        #记录训练历史
            ...
            if val_loss<best_val_loss:
                ...
                #best_model_path=os.path.join(results['model_path'], f'best_model--loss{loss_str}--dice{dice_str}.pth')   
    ...
    if is_master_process:
        ...
        dice_str = f"{best_val_dice:.3f}".replace('.', 'p')
        final_model_path=os.path.join(results['model_path'],f'final_model--loss{loss_str}--dice{dice_str}.pth')
